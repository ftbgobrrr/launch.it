...
public class GameInstance extends Thread implements Runnable {
    ...
    private List<String> jvmArguments(List<String> jsonArguments) {
        ...
        try {
            ...
            File tempDirFile = new File(this.getManager().getLaunchit().getLauncherManager().getLauncherFile().getParentFile(), "bin/" + UUID.randomUUID().toString());
    ...
    private void unpackNatives(File targetDir) throws IOException {
        ...
        for (Library library : libraries) {
            if (!library.matchEnvironement() || library.getNatives() == null || library.getNatives().size() == 0)
                continue;
